...
public enum BlockManager {
	...
	NEWDOUBLEWOODSLAB(Stuff.newslabWoodDouble, true) {
		...
		protected void prepare() {
			...
			// We can not create the slab reciepe till after we have created both the single
			// and double slabs
	...
	REDROCKSLAB(Stuff.slabRedRock, false) {
		...
		protected void prepare() {
			...
			proxy.registerOre("cobblestoneSlab", thisBlock);
		}
	},
	...
	FENCE(Stuff.fence, false) {
		...
		protected void prepare() {
			...
			proxy.registerOreInAllSubblocks("fenceWood", thisBlock);
	...
	ACACIADOOR(Stuff.doorAcacia, true) {
		...
		protected void prepare() {
			...
			proxy.registerOre("doorWood", thisBlock);
	...
	JAPANESEMAPLEDOOR(Stuff.doorJapaneseMaple, true) {
		...
		protected void prepare() {
			...
			proxy.registerOre("doorWood", thisBlock);
			// proxy.registerItem(Stuff.doorJapaneseMapleItem.get(),
			// "item_door_japanesemaple");
	...
	RAINBOWEUCALYPTUSDOOR(Stuff.doorRainbowEucalyptus, true) {
		...
		protected void prepare() {
			...
			proxy.registerOre("doorWood", thisBlock);
			// proxy.registerItem(Stuff.doorRainbowEucalyptusItem.get(),
			// "item_door_rainboweucalyptus");
	...
	REDWOODDOOR(Stuff.doorRedwood, true) {
		...
		protected void prepare() {
			...
			// GameRegistry.registerItem(Stuff.paste.get(), "extrabiomes.paste",
			// Reference.MOD_ID);
	...
	ACACIAGATE(Stuff.gateAcacia, true) {
		...
		protected void prepare() {
			...
			proxy.registerOre("fenceGateWood", thisBlock);
	...
	CYPRESSGATE(Stuff.gateCypress, true) {
		...
		protected void prepare() {
			...
			// Add the recipe for the gate
			Extrabiomes.postInitEvent(new FenceGateActiveEvent(new ItemStack(Stuff.gateCypress.get(), 1),
