...
public class EBXLNormalSelector extends NewDawnBiomeSelector {
    ...
    public NewDawnBiome selectBiome(int blockX, int blockZ, ChunkInformation chunkInfo) {
        if (chunkInfo.isBelowGroundLevel(blockX, blockZ) || !chunkInfo.isHumidityMedium(blockX, blockZ)) return null;
        ...
        if (isForest) {
            if (isCold) {
                ...
                if (snowNoise > 0) {
                    if (chunkInfo.isMountain(blockX, blockZ)) return biomeMountainTaiga;
                    else if (snowNoise > 0.5) return biomeSnowForest;
                    else return biomePineForest;
                }
            } else if (isCool) {
                if (stretchAutumn.getNoise(blockX, blockZ) > 0) return biomeAutumnWoods;
            } else {
                if (chunkInfo.getAverageHeight() < 100 && stretchBirch.getNoise(blockX, blockZ) > 0) {
                    return biomeBirchForest;
                } else if (stretchForest.getNoise(blockX, blockZ) > 0) {
                    if (chunkInfo.getAverageHeight() > 100) return biomeForestedHills;
                    else return biomeWoodlands;
                }
            }
        } else {
            final double noise = stretchMeadow.getNoise(blockX, blockZ);
            if (noise > 0.6) return biomeShrubland;
            else if (noise > 0) return biomeMeadow;
        }
