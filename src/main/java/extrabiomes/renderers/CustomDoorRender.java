    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        ...
        if ((metadata & 8) == 0) {
            tessellator.setBrightness(
                renderer.renderMinY > 0.0D ? i1 : block.getMixedBrightnessForBlock(renderer.blockAccess, x, y - 1, z));
        ...
        if ((metadata & 8) != 0) {
            tessellator.setBrightness(
                renderer.renderMaxY < 1.0D ? i1 : block.getMixedBrightnessForBlock(renderer.blockAccess, x, y + 1, z));
