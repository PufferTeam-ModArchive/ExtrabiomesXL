...
public class WorldGenNewRedwood extends WorldGenAbstractTree {
    ...
    private boolean generateTree(World world, Random rand, int x, int y, int z) {
        ...
        /*
         * if (rand.nextInt(4) == 0) { b0 = 1; if (rand.nextInt(3) == 0) { b0 = 2; } } else { flag1 = 1; } //x-=1;
         * //z-=1; //for (int j1 = -b0; j1 <= b0; j1++) //{ // for (int j2 = -b0; j2 <= b0; j2++) // { //
         * if(world.getBlock(x + j1, y - 1, z + j2) == 0) { // return false; // } // } //} int b1 = b0; if (flag1 == 0)
         * { for (int j1 = -b0; j1 <= b0; j1++) { for (int j2 = -b0; j2 <= b0; j2++) { if (j1 == b0) {
         * setBlockAndNotifyAdequately(world, x + j1, y, z + j2, 0, 0); } int blockID; for (int k3 = 0; k3 <= height;
         * k3++) { blockID = world.getBlock(x + j1, y + k3, z + j2); if(Block.blocksList[blockID] == null ||
         * Block.blocksList[blockID].isLeaves(world, x + j1, y + k3, z + j2)) { setBlockAndNotifyAdequately(world, x +
         * j1, y + k3, z + j2, TreeBlock.BRANCH.getBlock(), TreeBlock.BRANCH.getMetadata()); } } if ((b0 !=
         * Math.abs(j1)) || (b0 != Math.abs(j2)) || (b0 == 1)) continue; for (int j3 = height; j3 >= 0; j3--) {
         * setBlockAndNotifyAdequately(world, x + j1, y + j3, z + j2, 0, 0); blockID = world.getBlock(x + j1, y + j3, z
         * + j2); if (!(j3 < height / 2) && (Block.blocksList[blockID] == null ||
         * Block.blocksList[blockID].canBeReplacedByLeaves(world, x + j1, y + j3, z + j2))) {
         * //setBlockAndNotifyAdequately(world, x + j1, y + j3, z + j2, TreeBlock.LEAVES.getBlock(),
         * TreeBlock.LEAVES.getMetadata()); } } if(world.getBlock(x + j1, y, z + j2) != 0) {
         * setBlockAndNotifyAdequately(world, x + j1, y - 1, z + j2, Block.dirt, 0); } else {
         * setBlockAndNotifyAdequately(world, x + j1, y - 1, z + j2, Block.grass, 0); } } } b0++; for (int k1 = -b0; k1
         * <= b0; k1++) { for (int k2 = -b0; k2 <= b0; k2++) { if ((b0 != Math.abs(k1)) && (b0 != Math.abs(k2))) {
         * continue; } for (int j4 = height / 2; j4 <= height; j4++) { if ((rand.nextInt(1) == 0) && (world.getBlock(x +
         * k1, y + j4, z + k2) == 0)) { //setBlockAndNotifyAdequately(world, x + k1, y + j4, z + k2,
         * TreeBlock.LEAVES.getBlock(), TreeBlock.LEAVES.getMetadata()); } } } } for (int j3 = height / 2; j3 <= height
         * - 3; j3++) { if (rand.nextInt(2) == 0) { generateBranch(world, rand, x - b1, y + j3, z, -1, 0); } if
         * (rand.nextInt(2) == 0) { generateBranch(world, rand, x + b1, y + j3, z, 1, 0); } if (rand.nextInt(2) == 0) {
         * generateBranch(world, rand, x, y + j3, z - b1, 0, -1); } if (rand.nextInt(2) == 0) { generateBranch(world,
         * rand, x, y + j3, z + b1, 0, 1); } if (rand.nextInt(2) == 0) { generateBranch(world, rand, x - b1, y + j3, z +
         * b1, -1, 1); } if (rand.nextInt(2) == 0) { generateBranch(world, rand, x - b1, y + j3, z - b1, -1, -1); } if
         * (rand.nextInt(2) == 0) { generateBranch(world, rand, x + b1, y + j3, z + b1, 1, 1); } if (rand.nextInt(2) ==
