...
public class EBXLAridSelector extends NewDawnBiomeSelector {
    ...
    public NewDawnBiome selectBiome(int blockX, int blockZ, ChunkInformation chunkInfo) {
        if (chunkInfo.isBelowGroundLevel(blockX, blockZ) || !chunkInfo.isHumiditySparse(blockX, blockZ)) return null;
        ...
        if (isCold) {
            if (isMountain) {
                if (stretchAlpine.getNoise(blockX, blockZ) > 0) return biomeAlpine;
                if (isFrigid && stretchGlacier.getNoise(blockX, blockZ) > 0) return biomeGlacier;
            } else {
                if (stretchTundra.getNoise(blockX, blockZ) > 0) return biomeTundra;
                else if (stretchWasteland.getNoise(blockX, blockZ) > 0) return biomeIceWasteland;
            }
        } else if (isMountain) {
            final double noise = stretchMountain.getNoise(blockX, blockZ);
            if (noise > 0.5) return biomeMountainDesert;
            else if (noise > 0) return biomeMountainRidge;
        } else if (stretchSavanna.getNoise(blockX, blockZ) > 0) {
            return biomeSavanna;
        } else if (stretchWasteland.getNoise(blockX, blockZ) > 0) return biomeWasteland;
