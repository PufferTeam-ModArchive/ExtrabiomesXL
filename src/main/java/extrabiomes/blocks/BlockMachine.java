...
public class BlockMachine extends Block {
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        ...
        blockIcon = iconRegister.registerIcon(Extrabiomes.TEXTURE_PATH + "genesis");
    }
    ...
    public boolean canDoGenesis(World world, int x, int y, int z, int range, EntityPlayer sender) {
        ...
        for (int x1 = chunkX.intValue() - range; x1 <= chunkX.intValue() + range; x1++) {
            for (int z1 = chunkZ.intValue() - range; z1 <= chunkZ.intValue() + range; z1++) {
                ...
                if (k < -short1 || k > short1 || l < -short1 || l > short1) {} else {
                    /*
                     * ChatMessageComponent message = new ChatMessageComponent();
                     * message.addText("[Project Lazareth] - You are to close to spawn!"); message.setBold(true);
                     * message.setColor(EnumChatFormatting.AQUA); sender.sendChatToPlayer(message);
                     */
    ...
    public void doGenesis(World world, int x, int y, int z, int range, EntityPlayer sender) {
        ...
        for (int x1 = chunkX.intValue() - range; x1 <= chunkX.intValue() + range; x1++) {
            for (int z1 = chunkZ.intValue() - range; z1 <= chunkZ.intValue() + range; z1++) {
                /*
                 * PlayerInstance chunkwatcher = worldObj.getPlayerManager().getOrCreateChunkWatcher(x1, z1, false); if
                 * (chunkwatcher != null) { chunkwatcher.sendToAllPlayersWatchingChunk(new
                 * Packet51MapChunk(worldObj.getChunkFromChunkCoords(x1, z1), true, -1)); }
                 */
            }
        }
        ...
        /*
         * LogHelper.info("GENESIS @ " + x + "," + y + "," + z + " by " + player.getDisplayName());
         * world.setBlockToAir(x, y, z); WorldServer worldServer = (WorldServer) world;
         * LogHelper.info(oldBiome.biomeName + " >> " + newBiome.biomeName); // update chunk's biome values Chunk chunk
         * = world.getChunkFromBlockCoords(x, z); byte[] chunkBiomes = chunk.getBiomeArray(); for (int k = 0; k <
         * chunkBiomes.length; ++k) { chunkBiomes[k] = (byte) ( newBiome.biomeID & 0xFF ); }
         * chunk.setBiomeArray(chunkBiomes); chunk.needsSaving(true); LogHelper.info("-1"); int chunkX =
         * chunk.xPosition; int chunkZ = chunk.zPosition; // TODO: move all players in the blast radius out of the way
         * List<EntityPlayerMP> players = Lists.newArrayList(); players.addAll(worldServer.playerEntities); for
         * (EntityPlayerMP p : players) { if( worldServer.getPlayerManager().isPlayerWatchingChunk(p, chunkX, chunkZ)) {
         * LogHelper.info(" - pushing " + p); p.setLocationAndAngles((chunkX - 5) << 4, p.posY, (chunkZ - 5) << 4, 0F,
         * 0F); worldServer.updateEntityWithOptionalForce(p, false); } } LogHelper.info("0"); // unload affected chunk
         * GenesisChunkProvider provider = new GenesisChunkProvider(world, newBiome);
         * provider.unloadChunksIfNotNearSpawn(chunkX, chunkZ); int loadCount = 0; while (provider.getLoadedChunkCount()
         * != loadCount) { loadCount = provider.getLoadedChunkCount(); provider.unloadQueuedChunks(); if (chunk != null)
         * { if( chunk.isChunkLoaded ) { LogHelper.warning("Failed to unload chunk!"); loadCount = 0; } else {
         * LogHelper.info("Chunk unloaded."); } chunk = null; // better safe than sorry } } LogHelper.info("1"); //
         * regenerate the chunk IChunkLoader loader = provider.getCurrentChunkLoader();
         * provider.setCurrentChunkLoader(null); provider.loadChunk(chunkX, chunkZ); provider.populate(provider, chunkX,
         * chunkZ); provider.setCurrentChunkLoader(loader); LogHelper.info("2"); PlayerInstance watcher =
         * worldServer.getPlayerManager() .getOrCreateChunkWatcher(chunkX, chunkZ, false); if (watcher != null) { chunk
         * = world.getChunkFromChunkCoords(chunkZ, chunkZ); Packet51MapChunk packet = new Packet51MapChunk(chunk, true,
         * -1); watcher.sendToAllPlayersWatchingChunk(packet); } else {
         * LogHelper.warning("Unable to find chunk watcher!"); } LogHelper.info("3");
         */
    }
