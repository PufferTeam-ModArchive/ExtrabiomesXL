...
public class BlockCropBasic extends BlockFlower {
    ...
    public IIcon getIcon(int side, int metadata) {
        ...
        try {
            ...
            LogHelper.warning("Unable to get stage IIcon for " + cropType.name() + " @ " + metadata);
            return null;
        }
    }
    ...
    /*
     * @Override public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float
     * px, float py, float pz) { final ItemStack item = player.inventory.getCurrentItem(); boolean bonemeal = false; if
     * (item != null && item.itemID == Item.dyePowder.itemID && item.getItemDamage() == 15) { bonemeal = true; } //
     * TODO: detect other types of fertilizer? if (bonemeal) { fertilize(world, x, y, z); }
     * super.onBlockActivated(world, x, y, z, player, side, px, py, pz); return true; }
     */
