...
public class EBXLDampSelector extends NewDawnBiomeSelector {
    ...
    public NewDawnBiome selectBiome(int blockX, int blockZ, ChunkInformation chunkInfo) {
        if (chunkInfo.isBelowGroundLevel(blockX, blockZ) || !chunkInfo.isHumidityWet(blockX, blockZ)) return null;
        ...
        if (isMountain) {
            if (stretchExtremeJungle.getNoise(blockX, blockZ) > 0) return biomeExtremeJungle;
        } else if (stretchGreen.getNoise(blockX, blockZ) > 0) {
            if (chunkInfo.isGroundLevelOrShallowWater(blockX, blockZ)) return biomeGreenSwamp;
            else return biomeGreenHills;
        } else if (stretchRedwood.getNoise(blockX, blockZ) > 0) {
            if (isLush) return biomeRedwoodLush;
            else return biomeRedwoodForest;
        } else if (isLush) {
            if (stretchMiniJungle.getNoise(blockX, blockZ) > 0) {
                return biomeMiniJungle;
            } else if (stretchRainforest.getNoise(blockX, blockZ) > 0) {
                if (isCold) return biomeSnowRainforest;
                else if (isTemperate) return biomeTemperateRainforest;
                else return biomeRainforest;
            }
        }
