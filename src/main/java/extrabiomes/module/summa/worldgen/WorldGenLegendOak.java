...
public class WorldGenLegendOak extends WorldGenerator {
    ...
    private enum TreeBlock {
        ...
        BRANCH(new ItemStack(Blocks.log)),
        TRUNK(new ItemStack(Blocks.log));
        ...
        private static void loadCustomBlocks() {
            if (Element.LOG_QUARTER_OAK.isPresent()) TRUNK.stack = Element.LOG_QUARTER_OAK.get();
    ...
    public boolean generate(World world, Random random, int x, int y, int z) {
        ...
        if (!TreeSoilRegistry.isValidSoil(world.getBlock(x, y - 1, z))) return false;
        ...
        for (int branch = 0; branch < 2; branch++) {
            ...
            while (length < size) {
                ...
                if (zDirection == BendDirection.STRAIGHT) {
                    ...
                    else x1 -= random.nextInt(2);
                    if (branch == 0) z1 += random.nextInt(2);
                    else z1 -= random.nextInt(2);
                }
                if (xDirection == BendDirection.STRAIGHT) {
                    if (zDirection == BendDirection.RIGHT) z1 += random.nextInt(2);
                    else z1 -= random.nextInt(2);
                    if (branch == 0) x1 += random.nextInt(2);
                    else x1 -= random.nextInt(2);
                }
                if (xDirection == BendDirection.RIGHT) {
                    if (zDirection == BendDirection.RIGHT) if (branch == 0) z1 += random.nextInt(2);
                    else x1 += random.nextInt(2);
                    if (zDirection == BendDirection.LEFT) if (branch == 0) z1 -= random.nextInt(2);
                    else x1 += random.nextInt(2);
                }
