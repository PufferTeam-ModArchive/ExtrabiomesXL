...
class WorldGenQuicksand extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int x1 = -2; x1 <= 2; x1++) for (int z1 = -2; z1 <= 2; z1++)
            if (world.isAirBlock(x + x1, y - 1, z + z1) && world.isAirBlock(x + x1, y - 2, z + z1)) return false;

        for (int x1 = -1; x1 <= 1; x1++) for (int z1 = -1; z1 <= 1; z1++)
            for (int y1 = -2; y1 <= 0; y1++) world.setBlock(x + x1, y + y1, z + z1, quicksand);
